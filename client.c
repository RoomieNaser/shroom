...
int main(int argc, char const *argv[]){
    ...
    char input_buffer[MAX_FILE_SIZE + 1024];
    ...
    while(1) {
        ...
        if (strncmp(input_buffer, "REGISTER ", 9) == 0) {
            char t_cmd[20], t_user[50], t_lbl[20];
            sscanf(input_buffer, "%s %s %s %s", t_cmd, t_user, t_lbl, my_key);
        }
        ...
        if (strlen(my_key) > 0 && strncmp(input_buffer, "SENDFILE", 8) != 0 && strncmp(input_buffer, "REGISTER", 8) != 0) {
            xor_cipher(input_buffer, send_len, my_key);
        }
        
        write(sock, input_buffer, send_len);
    }
